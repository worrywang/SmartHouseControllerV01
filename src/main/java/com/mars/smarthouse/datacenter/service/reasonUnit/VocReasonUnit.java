 ...
 */
public class VocReasonUnit extends EnvironmentReasonUnit {
    @Override
    public UnitReasonResult activating(List<Device> machineList) {
        String result = this.commonReason(machineList);
        String state = semanticAnnotator.VocAnnotator(result);
    ...
    protected void setComfortableLevel() {
        ...
        if (comfortable.equals(Lexicon.low)) {
            ...
            unitReasonResult.setComfortableLevel(Double.valueOf(df.format(Math.random() * 30.0)));
        }
    }
}
