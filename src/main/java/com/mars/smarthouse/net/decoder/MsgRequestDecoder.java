package com.mars.smarthouse.net.decoder;
...
public class MsgRequestDecoder extends ByteToMessageDecoder {
	...
	protected void decode(ChannelHandlerContext ctx, ByteBuf inputBuf, List<Object> out) throws Exception {
		...
		for(;;) {
			if (inputBuf.readableBytes() < Setting.HEAD_LENGTH) {  //这个HEAD_LENGTH表示头长度的字节数。        由于上面我们传的是一个int类型的值，所以这里HEAD_LENGTH的值为4.
				return;
			}
			inputBuf.markReaderIndex();          //标记当前的readIndex的位置
			...
			byte[] body = new byte[dataLength];  //  读取对应的长度，满足长度要求，把传送过来的数据，取出来
			inputBuf.readBytes(body);  //

			Object o = MsgProtocol.MsgRequest.parseFrom(body);  //将byte数据转化为我们需要的对象，采用protobuf序列化，自行选择
			out.add(o);
		}
	}
}
